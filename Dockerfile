...
RUN chown -R aario:aario /usr/sbin/mysqld && chmod u+x /usr/sbin/mysqld
...
ENTRYPOINT ["/etc/aa/entrypoint", "/usr/sbin/mysqld", "--defaults-file=/etc/aa/my.cnf", "--user=aario","--gtid-mode=ON", "--explicit_defaults_for_timestamp", "--enforce-gtid-consistency"]
